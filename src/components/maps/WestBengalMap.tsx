...
} from '../../data/westbengal-data';
import {
  samplePollingBooths,
  generateMockBooths
} from '../../data/polling-booths-sample';
import westBengalStateGeoJSON from '../../data/geo/westbengal-state.json';
import tamilNaduDistrictsGeoJSON from '../../data/geo/westbengal-districts.json';
import tamilNaduConstituenciesGeoJSON from '../../data/geo/westbengal-constituencies.json';
import L from 'leaflet';

interface WestBengalMapProps {
  height?: string;
  initialZoom?: number;
  className?: string;
}

export const WestBengalMap: React.FC<WestBengalMapProps> = React.memo(({
  height = '700px',
  initialZoom = 7.5,
  className = ''
}) => {
  ...
  const [mapCenter, setMapCenter] = useState<[number, number]>([
    westBengalState.center.lat,
    westBengalState.center.lng
  ]);
  const [mapZoom, setMapZoom] = useState<number>(initialZoom);
  const [currentGeoJSON, setCurrentGeoJSON] = useState<any>(westBengalStateGeoJSON);
  ...
  const getSentiment = useCallback((featureId: string): SentimentScore | undefined => {
    // Try direct lookup by code (e.g., 'WB03', 'WB001')
    ...
    // Try state codes
    if (featureId === 'WB') return westBengalState.sentiment;
    if (featureId === 'WB') return westBengalState.sentiment;
  ...
  const handleLevelClick = useCallback((level: 'state' | 'district' | 'constituency' | 'booth') => {
    if (level === 'state') {
      ...
      setCurrentGeoJSON(westBengalStateGeoJSON);
      setMapCenter([westBengalState.center.lat, westBengalState.center.lng]);
    ...
    <div className={`relative ${className}`}>
      ...
      >
        <h2 className="text-2xl font-bold text-gray-900">
          West Bengal & Pondicherry Electoral Map
        </h2>
      ...
      >
        ...
        <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-4 border border-orange-200">
          ...
          <div className="text-2xl font-bold text-orange-900">
            {westBengalState.sentiment?.positive}% Positive
          </div>
        </div>
      </motion.div>
    </div>
